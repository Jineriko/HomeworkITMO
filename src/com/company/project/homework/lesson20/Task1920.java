package com.company.project.homework.lesson20;
...
public class Task1920 {
    ...
    public static double task01(ArrayList<Article> articles) {
        ...
        List<OptionalDouble> avgAge = articles.stream() // создаем коллекцию для хранения среднего возраста для каждой статьи
                .map(list -> list.getAuthors().values() // получаем set из hashMap авторов
                        .stream() // преобразовываем set в поток
                        .mapToDouble(author -> LocalDate.now().getYear() - author.getBirth().getYear()) // получаем количество лет каждого автора
                        .average()) // находим среднее для каждого элемента set
